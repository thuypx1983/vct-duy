<?php 
if($row->ctrl&CATPRODUCT_CTRL_THAILAN){
	?>
	<li class="dropdown li_dropdown">
        ...
        <ul class="dropdown-menu menu_tour_ul_0" role="menu">
            <li role="presentation" class="dropdown-header">
			    <a  class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false" href="<?php echo urlBuild('tour-list.php',array('url'=>$row->urlrewrite)) ?>"><?php echo @T_TOUR_IN.' '.$row->name; ?><span class="caret"></span></a>
            </li>
					<?php 
					...
					<?php } ?>
				<?php 
				$rs_hotel = catproductlist(CATPRODUCT_CTRL_SHOW|CATPRODUCT_CTRL_HOTEL|CATPRODUCT_CTRL_THAILAN,null,true,null,100);
				if($hotel = mysql_fetch_object($rs_hotel)){
				?>
                    <li role="presentation" class="dropdown-header">
                        <a  class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false"  href="<?php echo urlBuild('hotel.php').'#'.$hotel->name?>"><?php echo @T_TOUR_IN.' '.$row->name; ?><span class="caret"></span></a>
                    </li>
					<?php 
				...
				while ($temp = mysql_fetch_object($rs_useful))
					if($temp->ctrl&CATNEWS_CTRL_THAILAN){
						$item = $temp;
						break;
					}
				if($item){
