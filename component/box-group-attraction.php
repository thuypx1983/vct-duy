							...
							while ($item = mysql_fetch_object($rs_sub)){
								if(@$item){
									...
									if($catnow->id==@$item->id)
										echo '<a href="'.urlBuild('attraction-list.php',array('url'=>@$item->urlrewrite)).'"><span>'.@$item->name.'</span></a>';
									else 
										echo '<a href="'.urlBuild('attraction-list.php',array('url'=>@$item->urlrewrite)).'">'.@$item->name.'</a>';
									echo '</li>';
									}
								}
							?>	
