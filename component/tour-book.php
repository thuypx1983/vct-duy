<div class="breadcrumbs clearfix">
    ...
    <div itemscope itemtype="http://data-vocabulary.org/Breadcrumb">
        <span itemprop="title"><?php echo T_TOUR_BOOK?></span>
    </div>
</div>
<div class="box-title">
    <div class="box-outer-2 box-right">
					...
					$name = $item->name.' ';
					if(@$item->quantity) $name.= $item->quantity.' '.@T_DAYS;
					if(@$item->unit) $name.= ' / '.$item->unit.' '.@T_NIGHTS;
					echo '<form method="post" action="process.php?act=tour-book" name="form_contact" onsubmit="return checkContactForm_4(this);">';
					$tag = array(
					'{{T_TOUR_NAME}}'=>$name,
					...
					'{{T_TOUR_START}}' => '<input type="text" class="hotel_book_checkin"  name="departdate">',
					'{{T_DURATION_TOUR}}'=>'<input type="text" class="contact_us" name="duration">',
					'{{T_STAR_HOTEL}}'=>'<select class="select_date_" name="star_hotel">'.@file_get_contents(PATH_APPLICATION.'hotelstar.htm').'</select>',
					...
					echo strtr(@file_get_contents(PATH_APPLICATION.'form-book-tour.htm'),$tag);
