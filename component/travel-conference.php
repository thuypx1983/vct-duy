<?php
$rs = catnewslist(CATNEWS_CTRL_SHOW|CATNEWS_CTRL_TRAVEL_CONFERENCE,null,1);
...
<Div class="patch_all">
	...
	<span class="patch_active">'.@T_TRAVEL_CONFERENCE.'</span>
</Div>';
if($cat){
?>
				...
				<div class="padding_hottour">
					<Div class="ct_tour_list_page_">
						...
						while ($item = mysql_fetch_object($rs)){
							$href = urlBuild('travel-conference-detail.php',array('url'=>$item->urlrewrite));
						?>
						...
						<Div class="page_next_tour_" align="right">
							<form method="get" action="<?php echo urlBuild('travel-conference.php')?>">
							<?php
								if($page<=$pagecount&&$page>1)
									echo '<A href="'.urlBuild('travel-conference.php',array('url'=>$cat->urlrewrite,'page'=>$page-1)).'"><<</A>';
								else if($page==1)	echo '<A href="'.urlBuild('travel-conference.php',array('url'=>$cat->urlrewrite,'page'=>$pagecount)).'"><<</A>';
								echo @T_SELECT_PAGE;
							?>	
							...
							<?php 
								if($page<$pagecount)
									...
									echo '<A href="'.urlBuild('travel-conference.php',array('url'=>$cat->urlrewrite,'page'=>1)).'">>></A>';
							?>
							</form>
						</Div>
						<?php }?>
