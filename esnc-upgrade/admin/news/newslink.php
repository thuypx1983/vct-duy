...
<table width="100%" cellpadding="0" cellspacing="0"  onClick="parent.document.getElementById('newitemmenu').style.visibility='hidden';">	
	...
	<tr>
		<th width="14%">&nbsp;</th>				
		<th width="63%" >Các nội dung liên quan đến  <?php echo '<strong>'.$this->name.'</strong>'; ?></th>		
	...
	<tr>		
		<td colspan="2">		
		<?php		
		//lay tin trong 1 nhom
		$rs = $this->getnews($cat['id'],$this->tag);
		$a = array();
		while($a[] = mysql_fetch_assoc($rs)); //cho danh sach bai vao mang de dem so tin bai trong nhom
		array_pop($a);
		mysql_free_result($rs);
		?>
		<div><DIV class="join_plus" onclick="showChildNodes(this);">&nbsp;</DIV><strong class="a"><?php echo $cat['name'].'</strong> (Có '.count($a).' bài viết)'; ?></div><div class="block">
			<table width="98%" cellpadding="0" cellspacing="0" class="tab_item">
				...
				//danh sach bai viet
				foreach($a as $row){	
					//lay trang thai link (ctrl) cua cac bai viet
		...
		<?php		
			foreach($a_tag as $tag){
				echo '<a href="'.URL_SELF.'?act='.ACT_SEARCH.'&NWid='.$this->newsid.'&CNid='.$cat['id'].'&NWlink='.$this->type.'&tag='.$tag.'">'.$tag.'</a>,';
			}				
		?>		
		</td>
		<?php }$i++;?>
