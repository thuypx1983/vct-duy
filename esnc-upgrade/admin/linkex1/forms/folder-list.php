<?php
$switch='&nbsp;';
switch($self_flag=$this->check()){
case CAT_FLAG_ITEM:
	mysql_close();
	redirect('item-list.php?'.$this->alias.'id='.$this->parentid);//this folder contain item, redirect to item list
	exit();
case CAT_FLAG_EMPTY:
case CAT_FLAG_ROOT:
	$switch="<a href=\"".URL_CWD."item-list.php?{$this->alias}id={$this->parentid}\" class=\"item folder_icon\">&nbsp;</a>";
	break;
}
...
<tbody>
       <tr id="iddlgNewGroup" style="display:none" height="24px">
				...
				<input type="text" class="text" name="name" size="40" style="border:1px solid #000000;width:auto" value="Nh&oacute;m m&#7899;i..." onFocus="clearTextBox(this);"  onClick="clearTextBox(this);"/>
                </td>
        ...
        <tr>
                ...
                <td class="folder_icon" onClick="navigateFolder(<?php echo $row['id'] ?>,<?php echo $row['flag'] ?>);">&nbsp;</td>
                <td >
				<input type="text" size="40" class="text" id="<?php echo "name{$row['id']}" ?>" style="border:1px solid #FFFFFF; width:auto; cursor:pointer<?php if (($row['ctrl']&1)==0) echo ";color:#CCCCCC"?>" value="<?php echo $row['name']; ?>" readonly="yes" onDblClick="navigateFolder(<?php echo $row['id'] ?>,<?php echo $row['flag'] ?>);" title="Nh&#225;y &#273;&#250;p chu&#7897;t &#273;&#7875; m&#7903;"/>
                </td>
                <td align="center" nowrap><span onClick="<?php echo "rename({$row['id']},this)"; ?>;" class="textbutton">T&ecirc;n</span><strong>|</strong><a href="<?php echo URL_CWD ?>folder.php?<?php echo "{$this->alias}id={$row['id']}"?>" class="item">Chi ti&#7871;t</a><strong>|</strong><a href="<?php echo URL_CWD ?>meta-folder.php?<?php echo "MTcatid={$row['id']}"?>" class="item">Meta</a><strong>|</strong><span <?php if($row['flag']==0) echo 'class="textbutton" style="color:#FF0000 " onClick="doDelGroup('.$row['id'].','.$row['flag'].');return false;"'; else echo 'class="text" style="color:#CCCCCC" ';?>>Xo&aacute;</span></td>
                <td align="center" style="cursor:pointer">
				<?php if(!($row['flag'] & CAT_FLAG_ITEM)) {?><a href="<?php echo URL_CWD."index.php?{$this->alias}parentid={$row['id']}" ?>" class="item folder_icon">&nbsp;</a>
				<?php } if(!($row['flag'] & CAT_FLAG_SUBCAT)) { ?> <a href="<?php echo URL_CWD."item-list.php?{$this->alias}id={$row['id']}" ?>" class="item item_icon">&nbsp;</a>
				<?php }?></td>
...
<script language="javascript" type="text/javascript">
<?php if($self_flag != CAT_FLAG_EMPTY){?>
url_newitem='#';
function doNewItem(){
...
function doNewItem(){
	...
	if (aself.indexOf('product')<0) {
		var url_newitem = '<?php echo URL_CWD?>item.php?<?php echo urlmodify($this->alias.'id',$this->parentid,'catid',$this->parentid); ?>';
...
var url_newitem = '<?php echo dirname(URL_SELF) ?>/item.php?<?php echo urlmodify($this->alias.'id',$this->parentid,'catid',$this->parentid); ?>';
...
<?php }
if($self_flag == CAT_FLAG_NOEXIST){?>
function doNew(){
	parent.banner.setStatus('L&#7895;i.Nh&oacute;m &#273;&atilde; b&#7883; xo&aacute;.H&atilde;y <img src="images/btn_leftrefresh.gif" align="absbottom"/> l&#7841;i c&acirc;y b&ecirc;n tr&aacute;i');
	return false;
}
<?php }?>
<?php dbclose();?>
