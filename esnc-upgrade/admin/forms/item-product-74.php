...
<tr>
	...
	<div><input type="text"  name="name" value="<?php echo $this->name; ?>" style="width:74%;" /></div>
	</td>
	<td>
		&nbsp;
	</td>
...
<tr>
	...
	<div style="float:left;width:49%;margin-left:5px;">
		<div class="title">Thu&#7897;c t&#224;u</div>
		<div>
		<?php 
			$rs = mysql_query("SELECT `a`.`id`,`a`.`name`,`a`.`ctrl` FROM `".DB_TABLE_PREFIX."product` as a WHERE `a`.`type`= ".PRODUCT_TYPE_CRUISE." AND `a`.`ctrl`&".PRODUCT_CTRL_SHOW); 
			echo '<select name="model" style="width: 150px">';
			echo '<option value="0">Ch&#7885;n t&#224;u</option>';
			while ($item = mysql_fetch_object($rs)){
				echo '<option value="'.$item->id.'" '.($item->id==htmlspecialchars($this->model)?' selected="selected"':'').'>'.$item->name.'</option>';
			}
			echo '</select>';
		?>
		</div>	
