<?php class catbanner{
	...
	function addrow(){
		global $sql;
		$sql='INSERT INTO `'.DB_TABLE_PREFIX.'catbanner`(`parentid`,`name`,`desc`,`view`,`ctrl`,`img1`,`alt1`,`extra`) VALUES ('
	...
	function updaterow(){
		global $sql;
		settype($this->id,'int');
		$sql = "UPDATE `".DB_TABLE_PREFIX."catbanner` SET"
	...
	function loadonerow(){
		global $sql;
		settype($this->id,'int');
		$sql = "SELECT `id`, `parentid`, `name`, `desc`, `view`, `ctrl`, `img1`, `alt1`, `extra` FROM `".DB_TABLE_PREFIX."catbanner` WHERE `id` = {$this->id}";
