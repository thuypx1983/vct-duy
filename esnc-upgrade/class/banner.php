<?php class banner{//Phai sua ham addrow va update bo dau phay (,) truoc field dau tien,
	...
	function addrow(){
		...
		.",'".mysql_real_escape_string(stripslashes($this->target))."'"
		.",'".mysql_format_datetime($this->expires)."'"
		.",'".(int)$this->click."'"
		.",'".mysql_real_escape_string(stripslashes($this->mybanner))."'"
		.",'".(int)$this->contactid."'"
		.')';
		if(mysql_query($sql)){
	...
	function updaterow(){
		...
		.",`Img`='".mysql_real_escape_string(stripslashes($this->img))."'"
		...
		.",`Target`='".mysql_real_escape_string(stripslashes($this->target))."'"
		.",`Expires`='".mysql_format_datetime($this->expires)."'"
//		.",`Click`='".(int)$this->click."'"
		.",`MyBanner`='".mysql_real_escape_string(stripslashes($this->mybanner))."'"
...
//			_trace($sql);
	...
	function isduplicated(){
		if(preg_match('/http:\/\/([^\"\']+)(?:\'|\"|$)/i',$this->desc.' '.$this->url,$ss)){
			...
			_trace($sql);
			#if(mysql_num_rows($rs=mysql_query($sql))){ //hanhnd@esnadvanced.com comment 21/11/2008
